...
Tensor = FloatTensor
#########################
def plot_graph(mean_episode_reward,max_episode,path):
    plt.figure(1)
    plt.clf()
    ...
    '''
    if len(episode_reward)==max_episode:
        np.savetxt(path+'episode-reward',episode_reward)
        np.savetxt(path+'mean-reward', means.numpy())
    '''
...
class dqn_net(nn.Module):
    ...
    def update(self,samples,loss_func,optim_func,learn_rate,target_net,BATCH_SIZE,GAMMA):
        ...
        '''
        torch.nn.utils.clip_grad_norm(parameters=self.parameters(), max_norm=10)
        '''
        print('after clip\n####################')
        for param in self.parameters():
            print(param.size(), 'max:', param.data.max(), 'min:', param.data.min(),'\n')
...
def learn(env,
    ...
    while episode_num < MAX_EPISODE:
        ...
        if time_step < 1000000:
            threshold=EPS_START-(a/1000000)*time_step
        else:
            threshold=EPS_END
        if time_step % ACTION_REPEAT==0:
            if prob <= threshold:
        ...
        ### do one step ###
        obs_next, reward, done, info = env.step(action_button)
        ...
        if done == False:
            ...
            mean_epi_reward.append(mean100)
            plot_graph(mean_epi_reward,MAX_EPISODE,DATAPATH)
...
if __name__=='__main__':
    ...
    learn(env=env,
        ...
        REPLAY_MEMORY_CAPACITY=10000,#1000000,
        BATCH_SIZE=32,
        LOSS_FUNCTION=nn.MSELoss,
        OPTIM_METHOD=optim.RMSprop,
        LEARNING_RATE=0.00025,
        GAMMA=0.99,
        NET_COPY_STEP=10000,#10000,
        OBSERVE=10000,#50000,
        TRAIN_FREQ=4,
        ACTION_REPEAT=4,
        MODELPATH='./data_save/dqn_pong/param.pt',

...
def dqn_brain(env,
    ...
    # build functions #
    ###################
    img_h,img_w,img_c=env.observation.shape
    ...
    q_c_values=q_net(obs_c_float,act_num,scope='q_net',reuse=True)
    ...
    optimizer=optimizer_spec.constructor(learn_rate=learn_rate,**optimizer_spec.kwargs)
    ...
    replay_buffer=utils.ReplayBuffer(replay_buffer_size,history_frames_num)
    #################
    # play the game #
    #################
    ...
    for t in itertools.count():
        ...
        if explore_prob<exploration.value(t):
            ...
            action_values=session.run(q_c_values,feed_dict={obs_c_ph:obs_c})
